...
PROCESS_VERSION = "v1.1.45-D156"   # ← AGENT: bump on every change  # D137-2: +GET /api/radar/active-markets  # D146-P1: /api/arb/health os.kill → is_process_alive()  # D148-3: +arb_router (/api/arb/health, /api/arb/stats)  # D150-1: +reconnect_critical field  # D152: no code change — investigation sprint  # D154: no backend change — entropy_window fix only  # D156-3: +GET /api/entropy/status (D153-4)
