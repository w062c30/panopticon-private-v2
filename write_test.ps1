# Create the test file using PowerShell here-string with single-quotes (literal string)
# This avoids any interpretation of special characters
...
class TestD56ConsensusTotal:
    def test_consensus_total_no_limit(self, tmp_path):
        ...
        conn.execute('CREATE TABLE discovered_entities (entity_id TEXT PRIMARY KEY, insider_score REAL NOT NULL, address TEXT, primary_tag TEXT DEFAULT '''' )')
...
$content | Out-File -FilePath 'tests/test_rvf_metrics.py' -Encoding UTF8
Write-Host "Written $([Math]::Round($content.Length / 1KB)) KB"